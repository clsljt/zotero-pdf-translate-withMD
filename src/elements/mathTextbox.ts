...
export class MathTextboxElement extends XULElementBase {
  ...
  private _updateOverlay(): void {
    ...
    const mathEnabled = (getPref("enableMathRendering") as boolean) === true;
    const mdEnabled = (getPref("enableMarkdownRendering") as boolean) === true;
    if ((!mathEnabled && !mdEnabled) || !this._value) {
      this._hideOverlay();
      return;
    }
    this._showOverlay();
  ...
  private _showOverlay(): void {
    ...
    overlay.innerHTML = renderMarkdown(this._value, this.ownerDocument);
